...
import 'package:grocery_store/widgets/new_item.dart';
...
class _GroceryListState extends State<GroceryList> {
  ...
  void _loadItems()async{
    final url = Uri.https(
    'flutter-prep-b83b1-default-rtdb.firebaseio.com', 'shopping-list.json');

  try {
    final response = await http.get(url);
  if (response.statusCode>=400) {
    setState(() {
          _error = 'Filed to fetch data. Please try again later.';
    });
  }
...
void _removeItem(GroceryItem item)async{
  ...
  final url = Uri.https(
    'flutter-prep-b83b1-default-rtdb.firebaseio.com', 'shopping-list/${item.id}.json');
